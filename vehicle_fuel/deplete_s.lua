...
function getFuelLoss( veh )
	...
	local effectiveModel = exports.vehicle:getEffectiveVehicleModel(veh)
	if getVehicleEngineState( veh ) and ( getElementData( veh, "fuel" ) or 0 ) > 0 and not fuellessVehicle[ effectiveModel ] then
...
function getBatteryGainLoss( veh )
	local effectiveModel = exports.vehicle:getEffectiveVehicleModel(veh)
	if fuellessVehicle[ effectiveModel ] then
		return 0
	else

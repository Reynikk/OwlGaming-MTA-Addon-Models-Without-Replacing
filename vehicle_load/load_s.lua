...
function loadOneVehicle(data, loadDeletedOne)
	...
	if type(data) == 'table' then
		...
		if not exports.vehicle:isValidVariant( data.model, var1, var2 ) then
			...
			dbExec( exports.mysql:getConn('mta'), "UPDATE vehicles SET variant1 = " .. var1 .. ", variant2 = " .. var2 .. " WHERE id='" .. exports.mysql:escape_string(data.id) .. "'")
		end

		-- Spawn the vehicle
		veh = exports.vehicle:createVehicleWithCustomModel(data.model, data.currx, data.curry, data.currz, data.currrx, data.currry, data.currrz, data.plate, false, var1, var2)
		if veh then
			-- pool allocation.

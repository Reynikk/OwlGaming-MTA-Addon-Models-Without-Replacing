...
function createPermanentVehicle(player, command, ...)
	if hasCreatePermission( player ) then
		...
		if not veh then
			outputChatBox("Invalid vehicle model specified in vehlib.", player, 255, 100, 100)
			return
		end
...
function vehicleRespawn(exploded)
	...
	local job = tonumber(getElementData(source, "job")) or 0
	...
	exports.anticheat:setEld( source, "fuel", exports.vehicle_fuel:getMaxFuel(getEffectiveVehicleModel(source)) )
...
function setRealInVehicle(thePlayer)
	if isVehicleLocked(source) then
		...
		local faction = tonumber(getElementData(source, "faction")) or -1
...
function removeFromFactionVehicle(thePlayer)
	...
	local job = tonumber(getElementData(thePlayer, "job")) or -1
	local seat = getPedOccupiedVehicleSeat(thePlayer)

...
function openEditVehicleWindow()
	...
	if exports.integration:isPlayerVehicleConsultant( localPlayer ) or exports.integration:isPlayerLeadAdmin( localPlayer ) or exports.integration:isPlayerVCTMember( localPlayer ) then
		...
		-- Check if vehicle has a unique record in database
		local existed = getElementData(theVehicle, 'unique')
		outputDebugString("VEHICLE MANAGER / CUSTOM HANDLING / Vehicle ID #" .. vehdbid .. ": Unique flag = " .. tostring(existed))

		-- For custom models, always enable unique handling button
		local originalModelID = getElementData(theVehicle, "custom_model_id") or getElementData(theVehicle, "vehicle:originalModel")
		local isCustomModel = exports.vehicle:isCustomVehicleModel(originalModelID or model)
		if isCustomModel then
			existed = true
			outputDebugString("VEHICLE MANAGER / CUSTOM HANDLING / Vehicle ID #" .. vehdbid .. ": Is custom model, forcing unique flag to true")
		end

...
function saveHandling(theVehicle1, saveToSQL, who)
	...
	-- Debug info
	local modelID = getElementModel(theVehicle1)
	local originalModelID = getElementData(theVehicle1, "custom_model_id") or getElementData(theVehicle1, "vehicle:originalModel")
	local isCustomModel = exports.vehicle:isCustomVehicleModel(originalModelID or modelID)
	outputDebugString("VEHICLE MANAGER / HANDLING / Saving handling for vehicle: Model=" .. modelID .. ", Original model=" .. tostring(originalModelID) .. ", Is custom: " .. tostring(isCustomModel))
	...
	elseif saveToSQL == 1 then --SAVE TO 1 INSTANCE (UNIQUE)
		...
		if not dbid or not tonumber(dbid) then
			outputDebugString("saveHandling failed: No dbid found")
			return false
		end

		-- Get original model ID for custom vehicles
		local originalModelID = getElementData(theVehicle1, "custom_model_id") or getElementData(theVehicle1, "vehicle:originalModel")
		local modelID = getElementModel(theVehicle1)
		local isCustomModel = exports.vehicle:isCustomVehicleModel(originalModelID or modelID)
		outputDebugString("VEHICLE MANAGER / HANDLING / Saving unique handling for vehicle ID #" .. dbid .. ": Model=" .. modelID .. ", Original model=" .. tostring(originalModelID) .. ", Is custom: " .. tostring(isCustomModel))
		...
		if isCustomModel then
			forumText = forumText .. "\n[INDENT]Custom Model ID:   [B]" .. tostring(originalModelID or modelID) .. "[/B][/INDENT]"
		end
		...
		if not row or not row.id then -- NOT EXISTED
			if not mysql:query_free("INSERT INTO `vehicles_custom` SET `id`='"..tostring(dbid).."', `handling`='"..toSQL(sHandling).."', `createdby`='"..getElementData(who, "account:id").."', `createdate`=NOW()  ")	then
				...
				if isCustomModel and originalModelID then
					mysql:query_free("UPDATE `vehicles_custom` SET `mtamodel`='"..tostring(originalModelID).."' WHERE `id`='"..tostring(dbid).."' ")
					outputDebugString("VEHICLE MANAGER / HANDLING / Stored custom model ID " .. tostring(originalModelID) .. " for vehicle ID #" .. dbid)
				end
			...
			if not mysql:query_free("UPDATE `vehicles_custom` SET `handling`='"..toSQL(sHandling).."', `updatedby`='"..getElementData(who, "account:id").."', `updatedate`=NOW() WHERE `id`='"..tostring(dbid).."' ")	then
				...
				if isCustomModel and originalModelID then
					mysql:query_free("UPDATE `vehicles_custom` SET `mtamodel`='"..tostring(originalModelID).."' WHERE `id`='"..tostring(dbid).."' ")
					outputDebugString("VEHICLE MANAGER / HANDLING / Updated custom model ID " .. tostring(originalModelID) .. " for vehicle ID #" .. dbid)
				end
...
function createTestVehicle(vehShopID, thePed, fromVehLib)
	...
	-- Check if it's a custom model and get the base model for type checking
	local typeCheckID = vehicleID
	if exports.vehicle:isCustomVehicleModel(vehicleID) then
		typeCheckID = exports.vehicle:getVehicleBaseModel(vehicleID)
		outputDebugString("VEHICLE MANAGER / HANDLING / Using base model " .. typeCheckID .. " for type checking of custom model " .. vehicleID)
	end

	if getVehicleType(typeCheckID) == "Trailer" then
	...
	local veh = exports.vehicle:createVehicleWithCustomModel(vehicleID, x, y, z , 0, 0, 180, plate)
	if not (veh) then
		outputChatBox("Invalid Vehicle ID.", client, 255, 0, 0)
		return false
	end

	-- Store custom model information if needed
	if exports.vehicle:isCustomVehicleModel(vehicleID) then
		exports.anticheat:changeProtectedElementDataEx(veh, "custom_model_id", vehicleID, true)
		exports.anticheat:changeProtectedElementDataEx(veh, "vehicle:originalModel", vehicleID, true)
		outputDebugString("Stored custom model ID " .. vehicleID .. " for vehicle")
	end
...
function loadHandlingToVeh(veh, handlingTable)
	...
	local originalModelID = getElementData(veh, "custom_model_id") or getElementData(veh, "vehicle:originalModel")
	local modelID = getElementModel(veh)
	local isCustomModel = exports.vehicle:isCustomVehicleModel(originalModelID or modelID)
	outputDebugString("VEHICLE MANAGER / HANDLING / Loading handling for vehicle ID #" .. dbid .. ": Model=" .. modelID .. ", Original model=" .. tostring(originalModelID) .. ", Is custom: " .. tostring(isCustomModel))

	-- Check if handling table is valid
	if type(handlingTable) ~= "string" or handlingTable == "" then
		outputDebugString("VEHICLE MANAGER / HANDLING / No handling table provided, loading default handling")

		-- If no handling is provided, try to load from database
		if dbid > 0 then
			local row = mysql:query_fetch_assoc("SELECT `handling` FROM `vehicles_custom` WHERE `id` = '" .. mysql:escape_string(dbid) .. "' LIMIT 1" ) or false
			if row and row.handling and row.handling ~= "" then
				handlingTable = row.handling
				outputDebugString("VEHICLE MANAGER / HANDLING / Loaded handling from database for vehicle ID #" .. dbid)
			else
				-- If no handling in vehicles_custom, try to load from vehicles_shop
				local shopID = getElementData(veh, "vehicle_shop_id")
				if shopID then
					...
					if shopRow and shopRow.handling and shopRow.handling ~= "" then
						handlingTable = shopRow.handling
						outputDebugString("VEHICLE MANAGER / HANDLING / Loaded handling from vehicle shop ID #" .. shopID .. " for vehicle ID #" .. dbid)
					end
				end
			end
		end

		-- If still no handling, return false
		if type(handlingTable) ~= "string" or handlingTable == "" then
			return false
		end
	end

	-- Parse handling table
	local h = fromJSON(handlingTable)
	if not h then
		outputDebugString("VEHICLE MANAGER / HANDLING / Failed to parse handling table")
		return false
	end

	-- Apply handling to vehicle
	if veh and isElement(veh) then
		-- For both standard and custom models, apply the handling directly
		-- This ensures that custom models get the same handling treatment as standard models
		outputDebugString("VEHICLE MANAGER / HANDLING / Applying handling to vehicle ID #" .. dbid)

		-- Apply each handling property
		for i = 1, #handlings do
			if i ~= 29 then -- Skip problematic handling property
				local value = h[i] or h[tostring(i)]
				if value ~= nil then
					setVehicleHandling(veh, handlings[i][1], value)
				end
			end
		end

		outputDebugString("VEHICLE MANAGER / HANDLING / Successfully applied handling to vehicle ID #" .. dbid)
		return true
	end

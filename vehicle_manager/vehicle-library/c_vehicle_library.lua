...
local function updateLibraryGrid(vehs)
	...
	for i = 1, #vehs do
		...
		guiGridListSetItemText(VehLibGrid, row, col.enabled, ((tostring(vehs[i].enabled) == "1") and "Yes" or "No"), false, true)
		-- Handle custom vehicle models (negative IDs)
		local modelId = tonumber(vehs[i].vehmtamodel)
		local modelName = ""
		if modelId and modelId < 0 then
			-- Custom model
			modelName = "Custom Vehicle ("..vehs[i].vehmtamodel..")"
		else
			-- Standard model
			modelName = getVehicleNameFromModel(modelId) or "Unknown"
			modelName = modelName.." ("..vehs[i].vehmtamodel..")"
		end
		guiGridListSetItemText(VehLibGrid, row, col.mtamodel, modelName, false, false)
...
function showLibrary(vehs, thePed)
	...
	if thePed and isElement(thePed) and getElementData(thePed, "carshop") then
		...
		-- Buy Vehicle button
		buttons["buyvehicle"] = guiCreateButton(0.148,0.9181,0.1237,0.0587,"Buy Vehicle",true,vehLib)
		guiSetFont(buttons["buyvehicle"],"default-bold-small")
		addEventHandler( "onClientGUIClick", buttons["buyvehicle"],
			function( button )
				if button == "left" then
					local row, col = -1, -1
					local row, col = guiGridListGetSelectedItem(VehLibGrid)
					if row ~= -1 and col ~= -1 then
						...
						guiSetText(vehLib, "You need to select a vehicle from the list above first.")
						playError()
						triggerServerEvent("shop:storeKeeperSay", localPlayer, localPlayer, "Which one do you want to buy?" , getElementData(thePed, "name"))
					end
				end
			end,
		false)

		buttons["ordervehicle"] = guiCreateButton(0.2844,0.9181,0.1237,0.0587,"Order Vehicle",true,vehLib)
...
function addNewVehicle(veh)
	...
	if veh.enabled and (tonumber(veh.enabled) == 1 or veh.enabled == true) then
		guiCheckBoxSetSelected(checkboxes[1], true)
		outputDebugString("VEHICLE MANAGER / VEHICLE LIB / addNewVehicle / Enabled: true")
	else
		outputDebugString("VEHICLE MANAGER / VEHICLE LIB / addNewVehicle / Enabled: false, value: " .. tostring(veh.enabled))
	end
...
function validateCreateVehicle(data)
	if guiGetText(buttons[9]) == "Create" or guiGetText(buttons[9]) == "Update" then
		...
		if checkboxes[1] and isElement(checkboxes[1]) and guiCheckBoxGetSelected(checkboxes[1]) then
			veh.enabled = true
			outputDebugString("VEHICLE MANAGER / VEHICLE LIB / validateCreateVehicle / Enabled: true")
		else
			veh.enabled = false
			outputDebugString("VEHICLE MANAGER / VEHICLE LIB / validateCreateVehicle / Enabled: false")
		end
		...
		-- Check for custom model ID (negative number)
		local isCustomModel = tonumber(input) and tonumber(input) < 0
		...
		elseif vehModel and tonumber(vehModel) then
			guiSetText(labels[1], "MTA Vehicle Model (OK!):")
			guiLabelSetColor(labels[1], 0, 255,0)
		elseif isCustomModel then
			guiSetText(labels[1], "Custom Vehicle Model (OK!):")
			guiLabelSetColor(labels[1], 0, 255,0)

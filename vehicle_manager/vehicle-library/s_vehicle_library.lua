...
addEventHandler("vehlib:searchLibrary", root, function(keyword)
	...
	-- Check if it's a custom model ID (negative number)
	local isCustomModel = tonumber(keyword) and tonumber(keyword) < 0
	if model then
		keyword = model
...
function createVehicleRecord(data)
	...
	if data.enabled then
		enabled = "1"
		outputDebugString("VEHICLE MANAGER / VEHICLE LIB / createVehicleRecord / Enabled: true")
	else
		outputDebugString("VEHICLE MANAGER / VEHICLE LIB / createVehicleRecord / Enabled: false")
	end

	-- Ensure data.mtaModel is properly formatted for both standard and custom models
	if tonumber(data.mtaModel) and tonumber(data.mtaModel) < 0 then
		-- It's a custom model (negative ID), keep it as is
		data.mtaModel = tostring(data.mtaModel)
		outputDebugString("VEHICLE MANAGER / VEHICLE LIB / createVehicleRecord / Custom model ID: " .. data.mtaModel)
	elseif tonumber(data.mtaModel) then
		-- It's a standard model (positive ID), keep it as is
		data.mtaModel = tostring(data.mtaModel)
	else
		-- It's a vehicle name, convert to ID
		local modelID = getVehicleModelFromName(data.mtaModel)
		if modelID then
			data.mtaModel = tostring(modelID)
		else
			outputChatBox("[VEHICLE MANAGER] Invalid vehicle model: " .. data.mtaModel, client, 255, 0, 0)
			return false
		end
	end
...
function getCurrentVehicleRecord(id)
	...
	if row then
		...
		outputDebugString("VEHICLE MANAGER / VEHICLE LIB / getCurrentVehicleRecord / Enabled: " .. tostring(row.enabled))
		veh.update = true
